"""Layer norm implementation in triton.
Copy from https://github.com/openai/triton/blob/master/python/tutorials/05-layer-norm.py
To enable autotune, uncomment the triton.autotune decorator, and make sure
you remove the corresponding arugments (e.g., BLOCK_SIZE) when calling these functions
(e.g., make sure you don't pass BLOCK_SIZE to _layer_norm_bwd_dx_fused).
"""
import numbers
...
class TritonLayerNorm(torch.nn.Module):
    ...
    def forward(self, input):
        if input.is_cuda and input.dtype == torch.float16 and self.elementwise_affine:
            return layer_norm_affine(input, self.normalized_shape, self.weight, self.bias, self.eps)
